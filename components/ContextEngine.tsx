...
import { Upload, Download, Search, CheckCircle, X, Loader2, Database, Plus, Trash2, ArrowRight, MapPin, Globe, FileText, Link, ShieldAlert, Sparkles, Sliders, Code, Zap } from 'lucide-react';
...
const ContextEngine: React.FC<ContextEngineProps> = ({ 
  ...
  const handleMunicipalitySearch = async () => {
    ...
    const results = await searchMunicipalitySources(searchQuery);
  ...
  const handleBuildEngine = async () => {
    ...
    const entries = await generateContextDictionary(combinedContext);
    setDictionary(prev => {
  ...
  const handleDrop = useCallback(async (e: React.DragEvent) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files[0]) {
       ...
       const entries = await generateContextDictionary(`Context derived from file: ${file.name}`);
                     ...
                     <div className="p-8 space-y-8 animate-fade-in">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="bg-stone-50 p-4 rounded-xl border border-stone-200">
                                ...
                                <p className="text-xs text-stone-500 mb-3">How strictly to match phonetic sounds.</p>
                             ...
                             <div className="bg-stone-50 p-4 rounded-xl border border-stone-200 flex justify-between items-center">
                                 <div>
                                     <h4 className="font-bold text-sm text-forest-dark">Acronym Expansion</h4>
                                     <p className="text-xs text-stone-500">Convert "EPA" to "E.P.A." or full name</p>
                                 </div>
                                 <div 
                                    onClick={() => setSettings({...settings, acronymExpansion: !settings.acronymExpansion})}
                                    className={`w-12 h-6 rounded-full cursor-pointer transition-colors relative ${settings.acronymExpansion ? 'bg-forest-dark' : 'bg-stone-300'}`}
                                 >
                                     <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${settings.acronymExpansion ? 'left-7' : 'left-1'}`}></div>
                                 </div>
                            </div>

                             {/* Dialect */}
                             <div className="bg-stone-50 p-4 rounded-xl border border-stone-200">
                                 <h4 className="font-bold text-sm text-forest-dark mb-2">Context Domain</h4>
                                 <select 
                                    value={settings.dialect}
                                    onChange={(e) => setSettings({...settings, dialect: e.target.value as any})}
                                    className="w-full bg-white border border-stone-200 rounded p-2 text-sm"
                                 >
                            ...
                            <div className="bg-stone-50 p-4 rounded-xl border border-stone-200 flex justify-between items-center">
                                 <div>
                                     <h4 className="font-bold text-sm text-forest-dark">Profanity Filter</h4>
                                     <p className="text-xs text-stone-500">Mask offensive language (***)</p>
                                 </div>
                                 <div 
                                    onClick={() => setProfanityFilter(!profanityFilter)}
                                    className={`w-12 h-6 rounded-full cursor-pointer transition-colors relative ${profanityFilter ? 'bg-forest-dark' : 'bg-stone-300'}`}
                                 >
                                     <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${profanityFilter ? 'left-7' : 'left-1'}`}></div>
                                 </div>
                            </div>
                        </div>
                     </div>
                    ...
                    <div className="p-8 h-full flex flex-col animate-fade-in">
                        ...
                        {view === 'wizard_select' && (
                             <div className="flex-1 flex flex-col">
                                ...
                                <p className="text-stone-500 text-sm mb-6">Select the content chunks that contain relevant local context.</p>
